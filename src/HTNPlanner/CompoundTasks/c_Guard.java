...
public class c_Guard extends CompoundTask
{
	//how often each method was used to decompose this task
		...
		public c_Guard()
		{
			...
			this.methods.add(new m_StandGuard());	
			this.methods.add(new m_CrouchGuard());
			this.methods.add(new m_AirGuard());
			
			c_Guard.methodsUsed = new int[this.methods.size()];
			c_Guard.methodsSucceeded = new float[this.methods.size()];
		}
		...
		{
			this.selected ++;
			c_Guard.methodsUsed[methodIndex]++;
			int amount = c_Guard.methodsUsed[methodIndex];
		...
		public float GetExplorationValForMethod(int methodIndex)
		{
			...
			double numSelectedMethod = (double)c_Guard.methodsUsed[methodIndex] + UCBPlanner.EPSILON;
		...
		public float GetExploitationValForMethod(int methodIndex)
		{
			...
			float numSelected = (float)c_Guard.methodsUsed[methodIndex] + UCBPlanner.EPSILON;
			float numSucceeded = c_Guard.methodsSucceeded[methodIndex]/maxReward;
		...
		public void IncreaseSuccessStatistics(float currentReward) 
		{
			...
			IncreaseSuccessStatistics(c_Guard.methodsSucceeded, this.indMethodSelected,  currentReward, this.succeeded);
			if(Helper.DEBUG_UCB_STATICTICS)
			{
				System.out.println("Method " + this.methods.get(this.indMethodSelected).name + " succeeded in task " + this.name + " " + c_Guard.methodsSucceeded[indMethodSelected] + "/" + this.succeeded + " times"
						+ " av reward of m: " + c_Guard.methodsSucceeded[indMethodSelected]/c_Guard.methodsUsed[indMethodSelected]);
		...
		protected void FlushMethodRewards()
		{
			...
			if(this.tempReward > c_Guard.maxReward)
			{
				c_Guard.maxReward = this.tempReward;
			}
		}
}
