...
public class c_MiddleAttack extends CompoundTask
{
	...
	public c_MiddleAttack()
	{
		super();
		
		this.name = "c_MiddleAttack";
		...
		{
			this.methods = Planner.INSTANCE.GetMiddleAttacks_sorted();
		}
		else
		this.methods = Planner.INSTANCE.GetMiddleAttacks();
		
		c_MiddleAttack.methodsUsed = new int[this.methods.size()];
		c_MiddleAttack.methodsSucceeded = new float[this.methods.size()];
	}
	...
	{
		this.selected ++;
		c_MiddleAttack.methodsUsed[methodIndex]++;
		int amount = c_MiddleAttack.methodsUsed[methodIndex];
	...
	public float GetExplorationValForMethod(int methodIndex)
	{
		...
		double numSelectedMethod = (double)c_MiddleAttack.methodsUsed[methodIndex] + UCBPlanner.EPSILON;
	...
	public float GetExploitationValForMethod(int methodIndex)
	{
		...
		float numSelected = (float)c_MiddleAttack.methodsUsed[methodIndex] + UCBPlanner.EPSILON;
		float numSucceeded = c_MiddleAttack.methodsSucceeded[methodIndex]/maxReward;
	...
	public void IncreaseSuccessStatistics(float currentReward) 
	{
		...
		IncreaseSuccessStatistics(c_MiddleAttack.methodsSucceeded, this.indMethodSelected,  currentReward, this.succeeded);
		if(Helper.DEBUG_UCB_STATICTICS)
		{
			System.out.println("Method " + this.methods.get(this.indMethodSelected).name + " succeeded in task " + this.name + " " + c_MiddleAttack.methodsSucceeded[indMethodSelected] + "/" + this.succeeded + " times"
					+ " av reward of m: " + c_MiddleAttack.methodsSucceeded[indMethodSelected]/c_MiddleAttack.methodsUsed[indMethodSelected]);
	...
	protected void FlushMethodRewards()
	{
		...
		if(this.tempReward > c_MiddleAttack.maxReward)
		{
			c_MiddleAttack.maxReward = this.tempReward;
		}
	}
}
