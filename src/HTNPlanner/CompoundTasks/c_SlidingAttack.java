...
public class c_SlidingAttack extends CompoundTask 
{
	...
	public c_SlidingAttack()
	{
		super();
		
		this.name = "c_SlidingAttack";
		...
		{
			this.methods = Planner.INSTANCE.GetSlidingAttacks_sorted();
		}
		else
		this.methods = Planner.INSTANCE.GetSlidingAttacks();

		c_SlidingAttack.methodsUsed = new int[this.methods.size()];
		c_SlidingAttack.methodsSucceeded = new float[this.methods.size()];
	}
	...
	{
		this.selected ++;
		int amount = (c_SlidingAttack.methodsUsed[methodIndex]++);
	...
	public void SetUCBValues(String[] values)
	{
		...
		{
			//System.out.println("SLideAttack");
	...
	{		
		...
		double numSelectedMethod = (double)c_SlidingAttack.methodsUsed[methodIndex] + UCBPlanner.EPSILON;
	...
	public float GetExploitationValForMethod(int methodIndex)
	{
		...
		float numSelected = (float)c_SlidingAttack.methodsUsed[methodIndex] + UCBPlanner.EPSILON;
		float numSucceeded = c_SlidingAttack.methodsSucceeded[methodIndex]/maxReward;
	...
	public void IncreaseSuccessStatistics(float currentReward) 
	{
		...
		IncreaseSuccessStatistics(c_SlidingAttack.methodsSucceeded, this.indMethodSelected,  currentReward, this.succeeded);
		if(Helper.DEBUG_UCB_STATICTICS)
		{
			System.out.println(currentReward + " Method " + this.methods.get(this.indMethodSelected).name + " succeeded in task " + this.name + ", total reward of method: " + c_SlidingAttack.methodsSucceeded[indMethodSelected] + " and task" + this.succeeded
					+ " av reward of m: " + c_SlidingAttack.methodsSucceeded[indMethodSelected]/c_SlidingAttack.methodsUsed[indMethodSelected]);
	...
	protected void FlushMethodRewards()
	{
		...
		if(this.tempReward > c_SlidingAttack.maxReward)
		{
			c_SlidingAttack.maxReward = this.tempReward;
		}
	}
}
