...
public class c_AirAttackForewards extends CompoundTask 
{
	...
	public c_AirAttackForewards()
	{
		super();
		
		this.name = "c_AirAttackForewards";
		
		this.methods = Planner.INSTANCE.GetAirAttacksForewards();

		c_AirAttackForewards.methodsUsed = new int[this.methods.size()];
		c_AirAttackForewards.methodsSucceeded = new float[this.methods.size()];
	}
	...
	{
		this.selected ++;
		int amount = (c_AirAttackForewards.methodsUsed[methodIndex]++);
	...
	{		
		...
		double numSelectedMethod = (double)c_AirAttackForewards.methodsUsed[methodIndex] + UCBPlanner.EPSILON;
	...
	public float GetExploitationValForMethod(int methodIndex)
	{
		...
		float numSelected = (float)c_AirAttackForewards.methodsUsed[methodIndex] + UCBPlanner.EPSILON;
		float numSucceeded = c_AirAttackForewards.methodsSucceeded[methodIndex]/maxReward;
	...
	public void IncreaseSuccessStatistics(float currentReward) 
	{
		...
		IncreaseSuccessStatistics(c_AirAttackForewards.methodsSucceeded, this.indMethodSelected,  currentReward, this.succeeded);
		if(Helper.DEBUG_UCB_STATICTICS)
		{
			System.out.println(currentReward + " Method " + this.methods.get(this.indMethodSelected).name + " succeeded in task " + this.name + ", total reward of method: " + c_AirAttackForewards.methodsSucceeded[indMethodSelected] + " and task" + this.succeeded
					+ " av reward of m: " + c_AirAttackForewards.methodsSucceeded[indMethodSelected]/c_AirAttackForewards.methodsUsed[indMethodSelected]);
	...
	protected void FlushMethodRewards()
	{
		...
		if(this.tempReward > c_AirAttackForewards.maxReward)
		{
			c_AirAttackForewards.maxReward = this.tempReward;
		}
	}
}
