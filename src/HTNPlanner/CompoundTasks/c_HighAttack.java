...
public class c_HighAttack extends CompoundTask
{
	...
	public c_HighAttack()
	{
		...
		{
			this.methods = Planner.INSTANCE.GetHighAttacks_sorted();
		}
		else
		this.methods = Planner.INSTANCE.GetHighAttacks();
		
		c_HighAttack.methodsUsed = new int[this.methods.size()];
		c_HighAttack.methodsSucceeded = new float[this.methods.size()];
	}
	...
	{
		this.selected ++;
		c_HighAttack.methodsUsed[methodIndex]++;
		int amount = c_HighAttack.methodsUsed[methodIndex];
	...
	public float GetExplorationValForMethod(int methodIndex)
	{
		...
		double numSelectedMethod = (double)c_HighAttack.methodsUsed[methodIndex] + UCBPlanner.EPSILON;
	...
	public float GetExploitationValForMethod(int methodIndex)
	{
		...
		float numSelected = (float)c_HighAttack.methodsUsed[methodIndex] + UCBPlanner.EPSILON;
		float numSucceeded = c_HighAttack.methodsSucceeded[methodIndex]/maxReward;
	...
	public void IncreaseSuccessStatistics(float currentReward) 
	{
		...
		IncreaseSuccessStatistics(c_HighAttack.methodsSucceeded, this.indMethodSelected,  currentReward, this.succeeded);
		if(Helper.DEBUG_UCB_STATICTICS)
		{
			System.out.println("Method " + this.methods.get(this.indMethodSelected).name + " succeeded in task " + this.name + " " + c_HighAttack.methodsSucceeded[indMethodSelected] + "/" + this.succeeded + " times"
					+ " av reward of m: " + c_HighAttack.methodsSucceeded[indMethodSelected]/c_HighAttack.methodsUsed[indMethodSelected]);
	...
	protected void FlushMethodRewards()
	{
		...
		if(this.tempReward > c_HighAttack.maxReward)
		{
			c_HighAttack.maxReward = this.tempReward;
		}
	}
}
