...
public class c_Backwards extends CompoundTask
{
	//how often each method was used to decompose this task
		...
		public c_Backwards()
		{
			...
			this.name = "c_Backwards";
			
			this.methods.add(new m_BackStep());
			this.methods.add(new m_BackJump());
	
			
			c_Backwards.methodsUsed = new int[this.methods.size()];
			c_Backwards.methodsSucceeded = new float[this.methods.size()];
		}
		...
		{
			this.selected ++;
			c_Backwards.methodsUsed[methodIndex]++;
			int amount = c_Backwards.methodsUsed[methodIndex];
		...
		public float GetExplorationValForMethod(int methodIndex)
		{
			...
			double numSelectedMethod = (double)c_Backwards.methodsUsed[methodIndex] + UCBPlanner.EPSILON;
		...
		public float GetExploitationValForMethod(int methodIndex)
		{
			...
			float numSelected = (float)c_Backwards.methodsUsed[methodIndex] + UCBPlanner.EPSILON;
			float numSucceeded = c_Backwards.methodsSucceeded[methodIndex]/maxReward;
		...
		public void IncreaseSuccessStatistics(float currentReward) 
		{
			...
			IncreaseSuccessStatistics(c_Backwards.methodsSucceeded, this.indMethodSelected,  currentReward, this.succeeded);
			if(Helper.DEBUG_UCB_STATICTICS)
			{
				System.out.println("Method " + this.methods.get(this.indMethodSelected).name + " succeeded in task " + this.name + " " + c_Backwards.methodsSucceeded[indMethodSelected] + "/" + this.succeeded + " times"
						+ " av reward of m: " + c_Backwards.methodsSucceeded[indMethodSelected]/c_Backwards.methodsUsed[indMethodSelected]);
		...
		protected void FlushMethodRewards()
		{
			...
			if(this.tempReward > c_Backwards.maxReward)
			{
				c_Backwards.maxReward = this.tempReward;
			}
		}
}
