...
public class c_KnockBackAttack extends CompoundTask 
{
	...
	public c_KnockBackAttack()
	{
		super();
		
		this.name = "c_KnockBackAttack";
		...
		{
			this.methods = Planner.INSTANCE.GetKnockBackAttacks_sorted();
		}
		else
		this.methods = Planner.INSTANCE.GetKnockBackAttacks();

		c_KnockBackAttack.methodsUsed = new int[this.methods.size()];
		c_KnockBackAttack.methodsSucceeded = new float[this.methods.size()];
	}
	...
	{
		this.selected ++;
		int amount = (c_KnockBackAttack.methodsUsed[methodIndex]++);
	...
	{		
		...
		double numSelectedMethod = (double)c_KnockBackAttack.methodsUsed[methodIndex] + UCBPlanner.EPSILON;
	...
	public float GetExploitationValForMethod(int methodIndex)
	{
		...
		float numSelected = (float)c_KnockBackAttack.methodsUsed[methodIndex] + UCBPlanner.EPSILON;
		float numSucceeded = c_KnockBackAttack.methodsSucceeded[methodIndex]/maxReward;
	...
	public void IncreaseSuccessStatistics(float currentReward) 
	{
		...
		IncreaseSuccessStatistics(c_KnockBackAttack.methodsSucceeded, this.indMethodSelected,  currentReward, this.succeeded);
		if(Helper.DEBUG_UCB_STATICTICS)
		{
			System.out.println(currentReward + " Method " + this.methods.get(this.indMethodSelected).name + " succeeded in task " + this.name + ", total reward of method: " + c_KnockBackAttack.methodsSucceeded[indMethodSelected] + " and task" + this.succeeded
					+ " av reward of m: " + c_KnockBackAttack.methodsSucceeded[indMethodSelected]/c_KnockBackAttack.methodsUsed[indMethodSelected]);
	...
	protected void FlushMethodRewards()
	{
		...
		if(this.tempReward > c_KnockBackAttack.maxReward)
		{
			c_KnockBackAttack.maxReward = this.tempReward;
		}
	}
}
