...
public class c_ProjectileAttack extends CompoundTask
{
	...
	public c_ProjectileAttack()
	{
		super();
		
		this.name = "c_ProjectileAttack";
		...
		{
			this.methods = Planner.INSTANCE.GetProjectileAttacks_sorted();
		}
		else
		this.methods = Planner.INSTANCE.GetProjectileAttacks();
		
		c_ProjectileAttack.methodsUsed = new int[this.methods.size()];
		c_ProjectileAttack.methodsSucceeded = new float[this.methods.size()];
	}
	...
	{
		this.selected ++;
		c_ProjectileAttack.methodsUsed[methodIndex]++;
		int amount = c_ProjectileAttack.methodsUsed[methodIndex];
	...
	public float GetExplorationValForMethod(int methodIndex)
	{
		...
		double numSelectedMethod = (double)c_ProjectileAttack.methodsUsed[methodIndex] + UCBPlanner.EPSILON;
	...
	public float GetExploitationValForMethod(int methodIndex)
	{
		...
		float numSelected = (float)c_ProjectileAttack.methodsUsed[methodIndex] + UCBPlanner.EPSILON;
		float numSucceeded = c_ProjectileAttack.methodsSucceeded[methodIndex]/maxReward;
	...
	public void IncreaseSuccessStatistics(float currentReward) 
	{
		...
		IncreaseSuccessStatistics(c_ProjectileAttack.methodsSucceeded, this.indMethodSelected,  currentReward, this.succeeded);
		if(Helper.DEBUG_UCB_STATICTICS)
		{
			System.out.println("Method " + this.methods.get(this.indMethodSelected).name + " succeeded in task " + this.name + ", total reward of method: " + c_ProjectileAttack.methodsSucceeded[indMethodSelected] + " and task" + this.succeeded
					+ " av reward of m: " + c_ProjectileAttack.methodsSucceeded[indMethodSelected]/c_ProjectileAttack.methodsUsed[indMethodSelected]);
	...
	protected void FlushMethodRewards()
	{
		...
		if(this.tempReward > c_ProjectileAttack.maxReward)
		{
			c_ProjectileAttack.maxReward = this.tempReward;
		}
	}
}
