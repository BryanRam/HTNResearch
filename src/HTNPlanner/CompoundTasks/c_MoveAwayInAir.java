...
public class c_MoveAwayInAir extends CompoundTask 
{
	...
	public c_MoveAwayInAir()
	{
		...
		c_MoveAwayInAir.methodsSucceeded = new float[this.methods.size()];
	}
	...
	{
		this.selected ++;
		int amount = (c_MoveAwayInAir.methodsUsed[methodIndex]++);
	...
	{		
		...
		double numSelectedMethod = (double)c_MoveAwayInAir.methodsUsed[methodIndex] + UCBPlanner.EPSILON;
	...
	public float GetExploitationValForMethod(int methodIndex)
	{
		...
		float numSelected = (float)c_MoveAwayInAir.methodsUsed[methodIndex] + UCBPlanner.EPSILON;
		float numSucceeded = c_MoveAwayInAir.methodsSucceeded[methodIndex]/maxReward;
	...
	public void IncreaseSuccessStatistics(float currentReward) 
	{
		...
		IncreaseSuccessStatistics(c_MoveAwayInAir.methodsSucceeded, this.indMethodSelected,  currentReward, this.succeeded);
		if(Helper.DEBUG_UCB_STATICTICS)
		{
			System.out.println(currentReward + " Method " + this.methods.get(this.indMethodSelected).name + " succeeded in task " + this.name + ", total reward of method: " + c_MoveAwayInAir.methodsSucceeded[indMethodSelected] + " and task" + this.succeeded
					+ " av reward of m: " + c_MoveAwayInAir.methodsSucceeded[indMethodSelected]/c_MoveAwayInAir.methodsUsed[indMethodSelected]);
	...
	protected void FlushMethodRewards()
	{
		...
		if(this.tempReward > c_MoveAwayInAir.maxReward)
		{
			c_MoveAwayInAir.maxReward = this.tempReward;
		}
	}
}
