...
public class c_Attack extends CompoundTask
{
	...
	{
		this.selected ++;
		c_Attack.methodsUsed[methodIndex]++;
		int amount = c_Attack.methodsUsed[methodIndex];
	...
	{		
		...
		double numSelectedMethod = (double)c_Attack.methodsUsed[methodIndex] + UCBPlanner.EPSILON;
	...
	public float GetExploitationValForMethod(int methodIndex)
	{
		...
		float numSelected = (float)c_Attack.methodsUsed[methodIndex] + UCBPlanner.EPSILON;
		float numSucceeded = c_Attack.methodsSucceeded[methodIndex]/maxReward;
	...
	public void IncreaseSuccessStatistics(float currentReward) 
	{
		...
		IncreaseSuccessStatistics(c_Attack.methodsSucceeded, this.indMethodSelected,  currentReward, this.succeeded);
		if(Helper.DEBUG_UCB_STATICTICS)
		{
			System.out.println("Method " + this.methods.get(this.indMethodSelected).name + " succeeded in task " + this.name + ", total reward of method: " + c_Attack.methodsSucceeded[indMethodSelected] + " and task" + this.succeeded
					+ " av reward of m: " + c_Attack.methodsSucceeded[indMethodSelected]/c_Attack.methodsUsed[indMethodSelected]);
	...
	protected void FlushMethodRewards()
	{
		...
		if(this.tempReward > c_Attack.maxReward)
		{
			c_Attack.maxReward = this.tempReward;
		}
	}
}
